...
export function ProductEditorScreen({ shopId, productId }: { shopId: string; productId?: string }) {
  ...
  const submit = async (input: ProductInput, image?: PickedImage) => {
    ...
    try {
      if (productId) await productsApi.update(shopId, productId, input, image);
        ...
        if (!image) throw new Error('Une image est obligatoire.');
        await productsApi.create(shopId, input, image);
      }
      router.back();
    } catch (reason) { setError(errorMessage(reason)); }
  ...
  return <>{error ? <Text style={styles.error}>{error}</Text> : null}<ProductForm initialProduct={product} requireImage={!productId} submitting={submitting} submitLabel={productId ? 'Enregistrer les modifications' : 'Créer le produit'} onSubmit={submit} /></>;
}
